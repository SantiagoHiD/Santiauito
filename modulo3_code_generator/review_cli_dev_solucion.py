"""review_cli_dev_solucion.py (M3-V4) — VERSION SOLUCION (uso docente, no subir al repo).

Misma estructura que review_cli_dev.py pero con los dos TODOs ya completos.
Sirve como referencia para mostrar en clase o para correr el HITL de un
Contract C V3 generado por las soluciones de V1+V2+V3.
...
def registrar_decision(
    ...
    """
    # Crear el registro auditable: quien, cuando, accion, sobre que, por que.
    # Este es el corazon de la auditabilidad CMMI L3.
    cambio = ReviewChange(
...
def aplicar_veredicto(
    ...
    #   al Modulo 4.
    """
    # Normalizar el veredicto a las 3 acciones validas.
    v = (veredicto or "").strip().lower()

    if v in ("aprobar", "aprobado", "approve", "approved"):
        review.review_status = ReviewStatus.APPROVED
        review.approved_by = reviewer
        review.approved_at = datetime.now()
        accion = "approved"
    elif v in ("rechazar", "rechazado", "reject", "rejected"):
        review.review_status = ReviewStatus.REJECTED
        accion = "rejected"
    else:
        # Default defensivo: cualquier cosa que no sea aprobar/rechazar -> cambios.
        review.review_status = ReviewStatus.NEEDS_CHANGES
        review.version += 1
        accion = "changes_requested"

    # Guardar el feedback libre del revisor sobre el Contract C completo.
    if feedback:
        review.reviewer_feedback = feedback

    # Auditable: el veredicto tambien se registra en el change_history.
    registrar_decision(
        review,
        reviewer=reviewer,
        action=accion,
